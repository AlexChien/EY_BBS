...
/*
	...
	$Id: my.inc.php 16688 2008-11-14 06:41:07Z cnteacher $
*/
...
if(empty($do)) {
	...
	if($do == 'fav') {
		...
		if(!empty($favid)) {
			...
			$query = $sdb->query("SELECT m.*, t.subject FROM {$tablepre}mythreads m, {$tablepre}threads t
			...
			$query = $sdb->query("SELECT m.*, t.subject FROM {$tablepre}myposts m, {$tablepre}threads t
					WHERE m.uid = '$discuz_uid' AND m.tid = t.tid ORDER BY m.dateline DESC LIMIT 0, 3");
